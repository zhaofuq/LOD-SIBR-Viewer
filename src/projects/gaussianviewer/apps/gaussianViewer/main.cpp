...
std::vector<fs::path> get_matching_filenames(const fs::path& path, const std::string& pattern) {
    std::vector<fs::path> matching_files;
    std::regex base_regex(pattern);
    
    if (!fs::exists(path) || !fs::is_directory(path)) {
        std::cerr << "Specified path does not exist or is not a directory." << std::endl;
        return matching_files;
    }

    for (const auto& entry : fs::directory_iterator(path)) {
        const auto& filename = entry.path().filename().string();
        if (std::regex_match(filename, base_regex)) {
...
int main(int ac, char** av) 
{
	...
	if (plyfile.back() != '/')
		plyfile += "/";
	plyfile += "point_cloud";
	const std::string pattern = "level_\\d+\\.ply";
	...
	std::vector<fs::path> plyfiles;
	if(renderMode=="gaussian"){
		plyfile += "/point_cloud.ply";
		plyfiles.push_back(plyfile);
	}else{
		plyfiles = get_matching_filenames(plyfile, pattern);
	}
	for (const auto& file : plyfiles) {
		SIBR_LOG << "Found PlY file: " << file << std::endl;
	}
	...
	// Create the ULR view.
	GaussianView::Ptr	gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfiles, &messageRead, sh_degree, white_background, !myArgs.noInterop, device, depth_max));

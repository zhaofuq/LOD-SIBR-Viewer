...
namespace sibr { 
	...
	public:
		 ...
		 */
		GaussianView(const sibr::BasicIBRScene::Ptr& ibrScene, uint render_w, uint render_h, const std::vector<fs::path>& files, bool* message_read, int sh_degree, bool white_bg = false, bool useInterop = true, int device = 0, float depth_max = 100.0f);
	...
	protected:
		...
		std::string currLevel = "Mixed";

		bool _cropping = false;
		sibr::Vector3f _boxmin, _boxmax;
		sibr::Vector3f _scenemin = sibr::Vector3f(FLT_MAX, FLT_MAX, FLT_MAX);
		sibr::Vector3f _scenemax = -_scenemin;
		char _buff[512] = "cropped.ply";

		bool _fastCulling = true;
		int _device = 0;
		int _sh_degree = 3;
		uint8_t _max_level = 1;
		float _depth_max = 100.0f;

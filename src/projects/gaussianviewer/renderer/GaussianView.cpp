	...
	uint8_t level)
{
	...
	int total_count=count + loaded_count;

	// Output number of Gaussians contained
	SIBR_LOG << "Loading " << total_count << " Gaussian splats" << std::endl;
	...
	{
		int i = mapp[k].second;
		pos[k+loaded_count] = points[i].pos;
		levels[k+loaded_count] = level;
		...
		for (int j = 0; j < 4; j++)
			rot[k+loaded_count].rot[j] = points[i].rot.rot[j] / length;

		// Exponentiate scale
		for(int j = 0; j < 3; j++)
			scales[k+loaded_count].scale[j] = exp(points[i].scale.scale[j]);

		// Activate alpha
		opacities[k+loaded_count] = sigmoid(points[i].opacity);

		shs[k+loaded_count].shs[0] = points[i].shs.shs[0];
		shs[k+loaded_count].shs[1] = points[i].shs.shs[1];
		shs[k+loaded_count].shs[2] = points[i].shs.shs[2];
		for (int j = 1; j < SH_N; j++)
		{
			shs[k+loaded_count].shs[j * 3 + 0] = points[i].shs.shs[(j - 1) + 3];
			shs[k+loaded_count].shs[j * 3 + 1] = points[i].shs.shs[(j - 1) + SH_N + 2];
			shs[k+loaded_count].shs[j * 3 + 2] = points[i].shs.shs[(j - 1) + 2 * SH_N + 1];
		}
	}
	return total_count;
}
...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr & ibrScene, uint render_w, uint render_h, const std::vector<fs::path>& files, bool* messageRead, int sh_degree, bool white_bg, bool useInterop, int device, float depth_max) :
	_scene(ibrScene),
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	for (const auto& file : files) {
		if (sh_degree == 0){
			count = loadPly<0>(file.string().c_str(), pos, shs, opacity, scale, rot, levels,  _scenemin, _scenemax, count, level);
		}
		else if (sh_degree == 1)
		{
			count = loadPly<1>(file.string().c_str(), pos, shs, opacity, scale, rot, levels,  _scenemin, _scenemax, count, level);
		}
		else if (sh_degree == 2)
		{
			count = loadPly<2>(file.string().c_str(), pos, shs, opacity, scale, rot, levels,  _scenemin, _scenemax, count, level);
		}
		else if (sh_degree == 3)
		{
			count = loadPly<3>(file.string().c_str(), pos, shs, opacity, scale, rot, levels,  _scenemin, _scenemax, count, level);
		}
		level+=1;
	}
	...
	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&level_cuda, sizeof(uint8_t) * P));
	CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(level_cuda, levels.data(), sizeof(uint8_t) * P, cudaMemcpyHostToDevice));
...
void sibr::GaussianView::onRenderIBR(sibr::IRenderTarget & dst, const sibr::Camera & eye)
{
	...
	else
	{
		...
		renderCount = CudaRasterizer::Rasterizer::forward(
			geomBufferFunc,
			binningBufferFunc,
			imgBufferFunc,
			count, _sh_degree, 16, _max_level, 
			background_cuda,
...
void sibr::GaussianView::onGUI()
{
	...
	if(ImGui::BeginCombo("Level", std::to_string(renderLevel).c_str())){
		for(uint8_t i; i < _max_level; i++){
			std::string label = "Level " + std::to_string(i);
			if (ImGui::Selectable(label.c_str()))
				renderLevel = i;
		}
		if (ImGui::Selectable("Level Mixed"))
			renderLevel = -1;
		ImGui::EndCombo();
	}
	...
	ImGui::Text("Total Gaussians: %d", count);
	ImGui::Text("Rendered Gaussians: %d", renderCount);
